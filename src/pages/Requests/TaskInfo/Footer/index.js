...
export default function Footer({
  ...
  async function acceptOrder() {
    setLoading("accept");
    await api.post(`/approve/order/${taskId}`).catch((error) => {
      if (error) {
        setLoading("");
        reloadTask();
        loadRequests();
  ...
  async function deliveryOrder() {
    setLoading("delivery");
    await api.post(`/onTheWay/order/${taskId}`).catch((error) => {
      if (error) {
        setLoading("");
        reloadTask();
        loadRequests();
  ...
  async function cancelOrder() {
    setLoading("cancel");
    await api.post(`/restaurant/cancel/${taskId}`).catch((error) => {
      if (error) {
        setLoading("");
        reloadTask();
        loadRequests();
